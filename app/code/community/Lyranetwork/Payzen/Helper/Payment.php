...
class Lyranetwork_Payzen_Helper_Payment extends Mage_Core_Helper_Abstract
{
    ...
    public function deleteIdentifier($attribute, $maskedAttribute)
    {
        ...
        try {
            if ($this->_getRestHelper()->getPassword()) {
                ...
                $this->_getHelper()->log("Identifier for customer {$customer->getEmail()} cannot be deleted on gateway: private key is not configured. Let's just delete it from OpenMage.");
            }

            // Delete identifier from OpenMage.
            ...
            if (in_array($e->getCode(), $invalidIdentCodes)) {
                ...
                $this->_getHelper()->log(
                    "Identifier for customer {$customer->getEmail()} is invalid or doesn't exist. Let's delete it from OpenMage",
                    Zend_Log::WARN
                );
    ...
    /**
     * Convert gateway transaction statuses to openmage transaction statuses
     *
