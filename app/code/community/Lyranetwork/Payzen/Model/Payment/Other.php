...
class Lyranetwork_Payzen_Model_Payment_Other extends Lyranetwork_Payzen_Model_Payment_Abstract
{
     ...
     *
     * @return array[string][array] An array "$code => $title" of added means
     */
    public function getAddedMeans()
    {
        $added_means = unserialize($this->getConfigData('added_payment_means'));

        $means = array();
        if (is_array($added_means) && ! empty($added_means)) {
            foreach ($added_means as $mean) {
                $means[$mean['code']] = $mean['title'];
            }
        }
