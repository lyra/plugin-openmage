...
abstract class Lyranetwork_Payzen_Model_Payment_Abstract extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getFormFields($order)
    {
        ...
        // Available_languages is given as csv by openmage.
        ...
        // Set other data specific to Oney 3x 4x payment ond risk assessment module.
    ...
    public function acceptPayment(Mage_Payment_Model_Info $payment)
    {
        ...
        // Client did not configure private key in module backend, let OpenMage offline accept payment.
        if (! $this->_getRestHelper()->getPassword($storeId)) {
            $this->_getHelper()->log("Cannot get online payment information for order #{$order->getIncrementId()}: private key is not configured, let OpenMage accept payment.");
            return true;
        }
        ...
        try {
            ...
            if ($e->getCode() === 'PSP_100') {
                ...
                $this->_getHelper()->log("Cannot get online payment information for order #{$order->getIncrementId()}: REST API not available for merchant, let OpenMage accept the payment.");
    ...
    public function denyPayment(Mage_Payment_Model_Info $payment)
    {
        ...
        // Client did not configure private key in module backend, let OpenMage offline cancel payment.
        if (! $this->_getRestHelper()->getPassword($storeId)) {
            $this->_getHelper()->log("Cannot online cancel payment for order #{$order->getIncrementId()}: private key is not configured, let OpenMage cancel payment.");

            $this->_getAdminSession()->addWarning(__('Payment is cancelled only in OpenMage. Please, consider cancelling the payment in PayZen Back Office.'));
            return true;
        }
        ...
        try {
            ...
            return true; // Let OpenMage cancel order.
        } catch (UnexpectedValueException $e) {
           ...
           Mage::throwException($e->getMessage());
            ...
            if ($e->getCode() === 'PSP_100') {
                // Merchant does not subscribe to REST WS option, accept payment offline.
                $this->_getHelper()->log("Cannot cancel payment online for order #{$order->getIncrementId()}: REST API not available for merchant, let OpenMage cancel the payment.");
    ...
    public function validatePayment(Mage_Payment_Model_Info $payment)
    {
        ...
        try {
            ...
            foreach ($uuidArray as $uuid) {
                ...
                // Load OpenMage payment transaction object.
                $txn = Mage::getModel('sales/order_payment_transaction')->setOrderPaymentObject($payment)
                    ->loadByTxnId($transId);
    ...
    protected function _validatePaymentOffline($order, $payment) {
        $this->_getAdminSession()->addWarning(__('Payment is validated only in OpenMage. Please, consider validating the payment in PayZen Back Office.'));
    ...
    protected function _getUserInfo()
    {
        ...
        $commentText = 'OpenMage user: ' . $user->getUser()->getUsername();
